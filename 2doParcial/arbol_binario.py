...
class BinaryTree:
    ...
    def cont(self, value):
        def __contador(root, value):
            cont = 0
            if root is not None:
                if root.value == value:
                    cont = 1
                cont += __contador(root.left, value)
                cont += __contador(root.right, value)
            return cont
        return __contador(self.root, value)

    def contador_heroes(self, is_hero):
        def __contador_heroes(root, is_hero):
            cont = 0
            if root is not None:
                if root.other_values is is_hero:
                    cont = 1
                cont += __contador_heroes(root.left, is_hero)
                cont += __contador_heroes(root.right, is_hero)
            return cont
        return __contador_heroes(self.root, is_hero)

    def contador_nodos(self):
        def __contador_nodos(root):
            cont = 0
            if root is not None:
                cont = 1
                cont += __contador_nodos(root.left)
                cont += __contador_nodos(root.right)
            return cont
        return __contador_nodos(self.root)
    ...
    def search_by_coincidence_proximity(self, value):
        def search_by_coincidence(root, value):
            if root is not None:
                if root.value.startswith(value):
                    print(f"Number: {root.value} - Name: {root.other_values[0]} - Type: {root.other_values[1]}")
                search_by_coincidence(root.left, value)
                search_by_coincidence(root.right, value)

        search_by_coincidence(self.root, value)
        
    def search_number_pokemon(self, key):
        def __search(root, key):
            if root is not None:
                if root.value == key:
                    print(f"Number: {root.value} - Name: {root.other_values[0]} - Type: {root.other_values[1]}")
    ...
    def by_level_name(self):
        if self.root is not None:
            ...
            while cola_tree.size() > 0:
                node = cola_tree.atention()
                print(f"Name: {node.value}. Number: {node.other_values[0]}. Type: {node.other_values[1]}")
                if node.left is not None:
                    cola_tree.arrive(node.left)
                if node.right is not None:
                    cola_tree.arrive(node.right)
        ...
        def contador(root):
            ...
            if root is not None:
                if "Eléctrico" in root.value:
                    cont = 1
                cont += contador(root.left)
                cont += contador(root.right)
            return cont

        return contador(self.root)
    

    def pokemon_acero(self):
        def contador(root):
            cont = 0
            if root is not None:
